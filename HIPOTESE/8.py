...
# — NOVO: Funções de entrada e resposta — #
def injetar_entrada(fita, entrada):
    # Injeta um padrão simples na fita, sobrescrevendo os primeiros elementos
...
for t in range(N_CICLOS):
    ...
    # — NOVO: Mostrar respostas das fitas —
    print(f"Ciclo {t+1} respostas:")
    for i in range(N_FITAS):
        resposta = gerar_resposta(fitas[i])
        print(f"  Fita {i}: {resposta}")
    print("-" * 30)
...
plt.figure(figsize=(8,5))
for i in range(N_FITAS):
    plt.plot(hist_ent[i], label=f"Fita {i} — C{i}: {labels[i]}")
